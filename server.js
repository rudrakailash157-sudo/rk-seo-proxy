...
app.get("/auth/callback", async (req, res) => {
  ...
  try {
    ...
    if (tokenData.access_token) {
      ...
      res.send(`<html><body style="font-family:sans-serif;background:#0D0500;color:#F5E6C8;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;"><div style="text-align:center;padding:40px;border:1px solid #D4A017;border-radius:12px;background:#1A0A00;"><div style="font-size:48px;margin-bottom:16px;">✅</div><h2 style="color:#F0C84A;margin-bottom:8px;">RudraKailash SEO Agent Connected!</h2><p style="color:#9A7050;">Your Shopify store is now linked. You can close this tab and return to the SEO tool.</p></div></body></html>`);
    } else {
...
app.post("/ai/generate", async (req, res) => {
  ...
  console.log(`🤖 AI generate called. Key present: ${!!ANTHROPIC_KEY}, Key prefix: ${ANTHROPIC_KEY ? ANTHROPIC_KEY.substring(0,15) : 'NONE'}`);
