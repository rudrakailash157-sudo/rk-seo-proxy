...
// ─── Price/buying-intent keyword detection ──────────────────────────────────
// A keyword like "cost of 7 mukhi rudraksha" or "7 mukhi rudraksha price" is
// commercial-intent, not content-intent — the RULE that says "FAQ: word 2+
// questions using exact LONG-TAIL INTENT PHRASE phrasing" would otherwise
// instruct the writer to produce a question like "What is the cost of 7
// Mukhi Rudraksha?" with no guardrail against answering it with an invented,
// soon-to-be-stale rupee figure (the same failure mode MEASUREMENT ACCURACY
// RULE already prevents for bead size/weight). Splitting these out lets the
// prompt (1) keep them out of <h2>/<h3>/opening-paragraph placement, where a
// bare "Price" keyword would read like an ad rather than a product page, and
// (2) hand them a specific, safe instruction instead of the generic one.
const PRICE_INTENT_RE = /\b(price|prices|priced|pricing|cost|costs|costing|cheap|cheapest|buy|buying|purchase|kimat|keemat|kimmat)\b/i;

function splitPriceIntentKeywords(list) {
  const price = [], rest = [];
  (list || []).forEach(kw => (PRICE_INTENT_RE.test(kw) ? price : rest).push(kw));
  return { price, rest };
}
...
function stripHeadingSuffixes(html) {
  ...
  return html.replace(/(<h[23][^>]*>)([\s\S]*?)(<\/h[23]>)/gi, (match, openTag, inner, closeTag) => {
    ...
    if (pipeCount < 2) return match; // leave normal headings untouched
    const cleaned = inner.split("|")[0].trim();
    if (!cleaned) return match; // never produce an empty heading
    console.warn(`⚠️  Stripped keyword-tag suffix from heading: "${inner.trim()}" → "${cleaned}"`);
...
async function runSEOPipeline(product) {
  ...
  if (competitors.length > 0) {
    try {
      ...
      try {
        ...
        const h1Split      = splitPriceIntentKeywords(extractedKeywords.h1);
        const h2h3Split    = splitPriceIntentKeywords(extractedKeywords.h2h3);
        const phrasesSplit = splitPriceIntentKeywords(extractedKeywords.phrases);
        const priceIntentKeywords = [...h1Split.price, ...h2h3Split.price, ...phrasesSplit.price];

        const h1List = h1Split.rest.slice(0,5).join(" / ");
        const h2h3List = h2h3Split.rest.slice(0,8).join(", ");
        const phraseList = phrasesSplit.rest.slice(0,8).join(", ");
        keywordBrief = [
          ...
          priceIntentKeywords.length ? `PRICE/COST/BUYING-INTENT KEYWORDS (handle per the PRICE ACCURACY RULE below — do NOT use as a heading or opening-paragraph keyword): ${priceIntentKeywords.join(", ")}` : "",
        ].filter(Boolean).join("\n");
  ...
  const kwPlacementInstructions = keywordBrief
    ? `MANDATORY KEYWORD PLACEMENT:\n${keywordBrief}\n\nPLACEMENT RULES:\n- <h2>: Must contain one of the H1 PRIMARY KEYWORDS\n- Opening <p>: Naturally include the primary H1 keyword within first 60 words\n- <h3> headings: Use H2/H3 SUB-TOPIC KEYWORDS as heading phrases where they fit\n- Bullets: Address LONG-TAIL INTENT PHRASES as seeker experience\n- FAQ: Word at least 2 questions using the exact phrasing of LONG-TAIL INTENT phrases\n- PRICE/COST/BUYING-INTENT KEYWORDS: address the underlying intent in ONE FAQ answer only, WITHOUT stating a specific rupee figure or number — explain what factors influence price (bead size, mukhi count, certification, silver capping if selected) and note that current price is shown in the price/variant selector on this page. Never invent, estimate, or state a specific price anywhere in the output — see PRICE ACCURACY RULE.\n\n${selfContainedRule}`
...
PRICE ACCURACY RULE (mandatory): Never state a specific price, cost figure, or rupee amount anywhere in the output — intro, bullets, or FAQ. Live pricing is already shown to the customer via the product's price/variant selector and a separate, deterministically-generated Pricing & Availability block built fresh from real Shopify data — this description must never duplicate or guess at a number, since prices vary by size/variant and change over time, and a stale or invented figure here would contradict the real price shown elsewhere on the page. If a price/cost-related keyword is supplied (see PRICE/COST/BUYING-INTENT KEYWORDS below, if present), address the underlying intent qualitatively — what factors influence price — rather than with a number.
